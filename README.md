...
### 切换股票
1. **右键** → **切换展示股票**
2. 子菜单显示所有已添加的股票列表，格式：**股票名称 (代码)  现价  涨跌幅**
   - 示例：`贵州茅台 (SH600519)  1701.50  +0.80%`（红色上涨）
   - 示例：`万科A (SZ000002)  8.32  -1.42%`（绿色下跌）
3. **左键点击**股票项：切换监控该股票（当前股票显示 ✓ 标记）
4. **右键点击**股票项：弹出确认框删除该股票
5. 价格信息每 **30 分钟**自动刷新一次（每只股票请求间隔 3 秒，防止接口限流）
6. 股票名称会自动保存到 `~/.stockview/config.json`，下次启动直接读取
